...
| `task_properties_correlation/` | Correlates 12 LLM-rated task properties (`m, d, s, r, h, e, t, tf, df, de, nt, ac`) from `data/final_eco_2025_with_task_properties.csv` against our 4 internal sources + 5 ANALYSIS_CONFIGS at major/minor/broad/occupation levels. Five scoring schemes: Composite A = `(d·m·s·h)/(r·tf·df)` — friction terms (`r`, `tf`, `df`) in the denominator since high values reduce automation likelihood; Composite B = `d·m·s` (slim variant); plus per-task weighting variants (× freq, × t, × freq×t, raw). Outputs: 4-panel stacked Spearman heatmap for the 12 single properties × freq (9 internal × 12 cols × 4 levels), 4-panel stacked heatmap for the 5 composite columns, 22-major × 12-property means heatmap (sorted by Composite A descending, no freq weighting). Headline: **`composite_A_time_a` (composite × `t` with no freq) is the strongest predictor we have** — ρ 0.49–0.59 across all 9 internal series at major level, every cell p < .05. The same composite × freq collapses to mean ρ ≈ 0.01 because `freq_mean` is dominated by physical/repetitive hourly tasks where our pct_tasks_affected is structurally low; `t` (per-instance duration, 1=mins → 5=weeks) inverts that and weights toward knowledge work. Per-property × freq cells are uniformly negative across all 12 properties × all internal series × all levels for the same freq-side-effect reason; friction properties (`r, tf, df, nt`) are the most negative (~ρ −0.4 at major), confirming the theoretical direction. 3 PNGs + 4 CSVs. |

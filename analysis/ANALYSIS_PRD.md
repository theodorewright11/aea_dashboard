...
|------------|--------------|
| `all_paper_charts/` | **Committed (gitignore exception).** Mirror of every figure currently in `analysis/paper/results/part_{1,2,3}/figures/`. `run.py` syncs the PNGs into `figures/part_{1,2,3}/`, drops a SKIP list of charts not in results.md, applies a CHART_ORDER per part (matches `paper/results/results.md`), and regenerates `all_paper_charts.md` (no-prose listing). Contains a re-excluded gitignored sub-folder `offshoots/` for trying alternate chart variants. Inside offshoots, `weighting_config_test/` (sub-folder) tests two paper-locking decisions: (Q1) all_confirmed vs. all_confirmed_conservative under freq weighting — surface finding shows ρ = 0.94 at occ level with national 40% → 33% / 61M → 51M / $4T → $3.5T, but a follow-up physical-filter audit (`physical_filter_audit.py`) reverses the recommendation: 41.9% of the 2,160 dropped (task, occ) pairs are in info majors, 223 are flagrantly mislabeled (d/m/s all above eco median), and the LLM physical classifier is over-flagging informational work like teller-computer entry and cashier checkout transactions. A "smart conservative" counterfactual (physical-flag + at-least-one-of-d/m/s above eco median gate) recovers 4.2pp of pct, 6.4M workers, and $0.3T wages from plain conservative — about 60% of plain conservative's cut. Revised recommendation: don't adopt plain conservative; choose between keeping regular all_confirmed, adopting smart-conservative, or improving the physical classifier first. (Q2) six per-task weightings on all_confirmed (freq, freq×t, freq×imp×rel, imp×rel, t, none) — top-4 majors stable across all 6, Spearman ρ ≥ 0.96 vs freq at occ level, mid-tier shuffles affect Education and Legal most, recommends keeping freq as primary. Ships a parallel ratio-of-totals compute pipeline (validated at ρ = 1.0000 vs the dashboard's `get_pct_tasks_affected`); `t` joined from `data/final_eco_2025_with_task_properties.csv`. |

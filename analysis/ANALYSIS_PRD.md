...
| `schaal_substitution/` | Replicates the chart types from `paper/results/part_1` + `part_2` (minus correlations and temporal) using Schaal 2025's per-task `auto_avg` (from `merged_tasks_full.csv`) in the position normally occupied by our pipeline's `auto_aug_mean`. Schaal score normalized as `auto_avg / 2` to match the original `auto_aug_mean / 5` 0–1 multiplier mechanic. Two versions per chart: **economy** (Schaal applied to every eco_2025 task that has a score, ~99% of eco rows) and **confirmed** (Schaal applied only to task-occ pairs that ALSO appear in the all_confirmed dataset, ~58% of eco rows). Six chart types replicated: overview (single A-vs-B paired bars), phys/info divide, job zone violin, GWA exposure ranking, major categories 3-panel, SKA levels (compute_ska piped with Schaal-derived pct). Eleven PNGs total. Headline: Schaal-economy at 35% pct_tasks vs. our All Confirmed at 40% — major-category rankings, phys/info divide, job zone gradient, and GWA pattern all reproduce, suggesting Part 2's structural findings survive the substitution. |

...
3. **SKA — Skills** (`ska_skills.png`): per-element bars for all Skills, normalized to % of workforce max so the gray "workforce max" bar always reads 100% and the AI Top-10 bar is the AI's capability share of that ceiling. Same scaling and column structure as the K&A chart. Red diamond = AI Max, red circle = AI P95 (no white outline). Black dot = workforce mean. X-axis: "Mean AI Capability as % of Workforce Max (AI Top-10 Average ÷ Occupation Max for the Element, Across Elements)", ticks shown.
4. **SKA — Knowledge & Abilities** (`ska_knowledge_abilities.png`): two stacked panels, each showing subcategory rollups (Knowledge: 10 O*NET subcategories, Abilities: 15 O*NET subcategories). Each cell = mean across the elements in that subcategory. Subcategory labels carry `(N subcategories | M elements)`. Same marker scheme as Skills (no white outlines). X range fixed to [0, 100]%.
5. **Work Activities** (`gwa_exposure.png`): all 37 GWAs ranked by % tasks exposed. Bar color encodes Workers Exposed via a vertical color-bar legend on the right. Right-side text uses `|` separators ("X workers exposed | $Y wages exposed").
6. **Major Categories** (`major_categories.png`): all 22 major occupational categories in 3 side-by-side panels (% Tasks Exposed / Workers Exposed / Wages Exposed).
...
4. **AI Usage Intensity by Sector** — chart 15 from `exploratory/pct_norm_vs_eco/run_v3.py`. Major occ categories ranked by Σ pct usage / Σ (freq × emp) over the full eco_2025 universe, anchored on Educational Instruction = 1.00×, dashed median line. Bar shading by % tasks exposed surfaced via a vertical colorbar legend on the right (replacing the inline gradient note in the subtitle). Subtitle says "Σ pct usage ÷ Σ (freq × employment)" and parenthetical notes "(work-related ChatGPT chats)". Imports v3 compute at function level.

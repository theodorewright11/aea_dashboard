...
| `action_levers/` | Six action-oriented charts on what to do about widespread AI exposure (independent of any chart in `questions/` or `paper/`). (1) Complementarity Quadrant — pct × human SKA headroom, sized by emp, colored by wage; identifies augmentation-OK vs substitution-risk vs vulnerable-when-reached jobs. (2) Sectoral Velocity Quadrant — current pct × annualized growth rate over the all_confirmed series, sized by workers in scope. (3) Wage Resilience Matrix — pct × log(median wage), sized by emp, colored by SKA gap; identifies wage cliff zones. (4) Resilience Differential — 3-panel ranking (Skills/Knowledge/Abilities) of element exposure premium between occupations that emphasize the element (importance ≥ 4) vs. those that don't. (5) Trade-Up Corridors — heatmap of avg L1 SKA distance from high-risk (top-quartile pct) to low-risk (bottom-quartile pct) major-major pairs, restricted to ±25% wage band. (6) Bottleneck Activity Atlas — per major sector, work-time share in residual (≤33%) / moderate (33–50%) / heavy (>50%) IWA exposure tiers. All charts use `all_confirmed` as primary lens. |

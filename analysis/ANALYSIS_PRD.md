...
| `action_levers/` | Persistent workspace for paper-bound charts and reports. Has its own `CLAUDE.md` agent spec, `master_report.md` rolling synthesis (single shareable deliverable, updated every session), `INVENTORY.md` meta log, `lib/` for shared helpers, and one named sub-folder per analysis iteration. Each sub-folder follows the same pattern (`run.py`, `results/`, `notes.md`, optional `<topic>_report.md`). The seed sub-folder `initial_action_charts/` produces six charts on what to do about widespread exposure: (1) Complementarity Quadrant — pct × human SKA headroom, sized by emp, colored by wage; (2) Sectoral Velocity Quadrant — current pct × annualized growth rate over the all_confirmed series; (3) Wage Resilience Matrix — pct × log(median wage), sized by emp, colored by SKA gap; (4) Resilience Differential — 3-panel ranking (Skills/Knowledge/Abilities) of element exposure premium; (5) Trade-Up Corridors — heatmap of avg L1 SKA distance from high-risk to low-risk major-major pairs within ±25% wage band; (6) Bottleneck Activity Atlas — per-sector work-time share in residual / moderate / heavy IWA exposure tiers. Future sub-folders extend or refine; promotion to `paper/` is gated by the user. |

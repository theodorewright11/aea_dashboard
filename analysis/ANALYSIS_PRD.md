...
1. **External Benchmark Comparison — by AI Source**: 2×2 grid of 4×10 heatmaps. Y-axis = our 4 internal sources (Claude Browser, Claude API, Copilot, MCP). X-axis = our 4 sources (lower-tri) + 6 external benchmarks (Eloundou GPT-4 β, Eloundou Human β, AIOE 10-app mean, AIOE Reading Comprehension, Schaal Overall, Schaal DA — both Schaal columns loaded from `analysis/data/Comparison of Indices.csv`). Single shared color bar `zmin = floor(min observed)`. Significance note ("All correlations significant at p<.001 (two-tailed Spearman)") is in the subtitle.
2. **External Benchmark Comparison — by Data Configuration**: 2×2 grid of 6×12 heatmaps. Y-axis = our 6 ANALYSIS_CONFIGS (now including `all_confirmed_conservative`). X-axis = 6 configs (lower-tri) + 6 external benchmarks (same as the source-level chart). Same color scale, methodology, and external rollup as the source-level chart.
3. **AI Economic Exposure Across Data Configurations** (the renamed "overview"): grouped horizontal bars per config, in Tasks → Workers → Wages order within each cluster. Includes the 6th config `all_confirmed_conservative`. Bar text: `36.1% tasks` / `61.3M (40.0%) workers` / `$4.0T (42.4%) wages`. Subtitle describes the chart at the same level as the other Part 1 figures ("Share of national task volume, employment, and wages exposed across the six AI data configurations.").
4. **Temporal**:
   - 3-panel growth chart (% Tasks Exposed / Workers Exposed / Wages Exposed). Per-panel metric color: tasks blue, workers gold, wages green. All Confirmed = solid line in primary metric color, All Sources (Ceiling) = dashed line in lighter shade. Per-point text labels only on start and end dates (mid-line points have markers but no text, so clustered dates don't crash labels into each other). Legend at the bottom shows one entry per series. Y-ranges tightened to data band, not zero-anchored.
   - Paired side-by-side per-config data table trimmed to Date / Unique Tasks Rated (with `(of N total in O*NET)` annotated under the column header where N is the total unique-task count from `load_eco_raw`) / Δ / AI Capability (+ Δ). Two cream historical rows (2024-09-30, 2024-12-23) prepended at the top of BOTH tables. Task counts on those rows come from the same combined dataset family the line chart uses (`AEI Both + Micro` for confirmed, `All` for ceiling) — Sep 2024 = 8,319 tasks (Microsoft only contributing on that date), Dec 2024 = 9,067 tasks (Microsoft + AEI Conv v1 combined). AI Capability is barred for those rows since the multi-source combination doesn't have enough coverage that early in the series.
...
1. **Physical/Informational Divide** (`phys_info_divide.png`): horizontal box plots of % tasks exposed for Physical (>67% physical tasks) / Mixed (33–67%) / Non-physical (<33%) occupation groups, ordered Physical → Mixed → Non-physical (top to bottom).
...
3. **SKA — Skills** (`ska_skills.png`): per-element bars for all Skills, normalized to % of workforce max so the gray "workforce max" bar always reads 100% and the AI Top-10 bar is the AI's capability share of that ceiling. Same scaling and column structure as the K&A chart. Red diamond = AI Max, red circle = AI P95 (no white outline). Black dot = workforce mean. Subtitle describes what the two bar colors represent (gray = workforce maximum; blue = AI Top-10 average as a share of that maximum). X-axis: "AI Capability as % of Workforce Max (AI Top-10 Average ÷ Occupation Max for the Element)", ticks shown.
4. **SKA — Knowledge & Abilities** (`ska_knowledge_abilities.png`): two stacked panels, each showing subcategory rollups (Knowledge: 10 O*NET subcategories, Abilities: 15 O*NET subcategories). Each cell = mean across the elements in that subcategory. Subtitle says the AI bar is "averaged across the subcategory". X-axis spells out the subcategory averaging: "AI Capability as % of Workforce Max — Average across the subcategory of (AI Top-10 ÷ Occupation Max per Element)". Subcategory labels carry `(N subcategories | M elements)`. Same marker scheme as Skills (no white outlines). X range fixed to [0, 100]%.
5. **Work Activities** (`gwa_exposure.png`): all 37 GWAs ranked by % tasks exposed, rendered at large size (PAPER_W + 1100, taller per-row pitch) so labels and right-side annotations don't crowd. Bar color encodes Workers Exposed via a vertical color-bar legend on the right. Right-side text uses `|` separators ("X workers exposed | $Y wages exposed").
6. **Major Categories** (`major_categories.png`): all 22 major occupational categories in 3 side-by-side panels (% Tasks Exposed / Workers Exposed / Wages Exposed). Subtitle and per-panel x-axis labels (with tick labels) on all three panels.

Narrative arc: physical/informational structure → preparation level gradient → element-level skills profile → subcategory-level knowledge/abilities profile → work activity rankings → sector-level view. Uses All Confirmed config throughout. Subtitles dropped the config-restating boilerplate (the methods section sets the primary config). All charts use the swapped-metric palette: tasks blue, workers gold, wages green-teal (money association). Axis labels are Title Case throughout the paper figures.

### Part 3 — Action: What To Do About It ("Here's what to do about it")

Four charts in narrative order:

1. **Conv → Confirmed → Ceiling exposure by major sector (top 10)** — top 10 majors by Conv→Confirmed % tasks gap, stacked into three segments: Conversational confirmed base (tasks-blue) + Conv → Confirmed agentic gap (focal, shaded by workers added) + Confirmed → Ceiling extension. Subtitle says "ceiling exposure" (not "ceiling reach"). Bar text inside segments shows +pp; right-side annotations show workers + wages deltas only (no pp). Workers-Added colorbar pushed far right (x=1.30 paper coords) so it doesn't overlap right-side annotation column.
2. **Top 25 O*NET Software Categories by Occupational Software Task Exposure × Employment** (`tech_commodities.png`) — formerly the "Tech commodities" chart. Y-axis is the O*NET Software Category, x-axis is "Software Workers In Scope", colorbar is "% Software Usage Exposed". Bar gradient is a workers-gold + tasks-blue blend (after METRIC_COLORS workers/wages swap), label order is `% avg | workers | wages | occs | entries`.
3. **Occupations Most At Risk Of Displacement (n=43)** — risk_score_audit Section 5f. Bars sorted by BLS projected employment decline 2024–2034 (absolute value). Bar shading = % tasks affected (tasks-blue gradient). Filter: pct > 50% AND pct trend > median AND emp proj < 0 AND SKA pct > median. Per-row label format: `+X.X% emp proj | YY% tasks | zone N | <major>`. Subtitle uses `%` (not `pp`) for the trend median. Imports `_load_flag_df` and `_build_focused_set` from the exploratory audit module at function level; skips gracefully if the audit folder isn't available.
4. **AI Usage Intensity by Sector** — chart 15 from `exploratory/pct_norm_vs_eco/run_v3.py`. Major occ categories ranked by Σ pct usage / Σ (freq × emp) over the full eco_2025 universe, anchored on Educational Instruction = 1.00×, dashed median line. Bar shading by % tasks exposed surfaced via a vertical colorbar legend on the right (replacing the inline gradient note in the subtitle). The pct_tasks_affected values that drive the colorbar are pulled from the same dashboard pipeline (`get_group_data` at major level on `all_confirmed`) that the Part 2 `major_categories` chart uses, so the colorbar range matches that chart exactly — NOT from `compute_major_pct_tasks_affected` (which restricts to rated task-occ pairs only and yields a different range). Subtitle says "Σ pct usage ÷ Σ (freq × employment)" and parenthetical notes "(work-related ChatGPT chats)". Imports v3 compute at function level.
...
|------------|--------------|
| `all_paper_charts/` | **Committed (gitignore exception).** Mirror of every figure currently in `analysis/paper/results/part_{1,2,3}/figures/`. `run.py` syncs the PNGs into `figures/part_{1,2,3}/`, drops a SKIP list of charts not in results.md, applies a CHART_ORDER per part (matches `paper/results/results.md`), and regenerates `all_paper_charts.md` (no-prose listing). Contains a re-excluded gitignored sub-folder `offshoots/` for trying alternate chart variants. Inside offshoots, `weighting_config_test/` (sub-folder) tests two paper-locking decisions through three scripts: (1) `run.py` does the original Stage A (regular vs all_confirmed_conservative) + Stage B (six weighting variants); (2) `physical_filter_audit.py` audits the LLM physical filter — 41.9% of plain conservative's drops are in info majors and 223 are flagrantly mislabeled (cashier transactions, teller computer entries, library checkouts), with a smart-conservative counterfactual recovering ~60% of plain conservative's cut; (3) `ai_filter_comparison.py` evaluates the user-built AI-share IWA filter (`final_all_confirmed_usage_ms_ai_phys_filtered_2026-02-12.csv`) — strips Microsoft contributions only on IWAs where AI share > 2× human share, drops just 305 pairs (vs 2,160) all of which are a strict subset of plain conservative's drops, with d/m/s of new drops matching the eco physical baseline almost exactly (d=2.91 vs 2.92). National totals: regular 40% / 61M / $4T → new AI-filter 38.5% / 58.9M / $3.9T → plain conservative 33% / 51M / $3.5T. Spearman ρ vs regular at occ level: new=0.992, plain=0.938. **Final recommendations: (Q1) adopt the new AI-share filter as primary All Confirmed and collapse to 5 configs (deprecate all_confirmed_conservative); (Q2) keep `freq` as the per-task weight (all six variants ρ ≥ 0.96 vs freq, top-4 majors stable across all).** Ships a parallel ratio-of-totals compute pipeline (validated at ρ = 1.0000 vs the dashboard's `get_pct_tasks_affected`); `t` joined from `data/final_eco_2025_with_task_properties.csv`. |
...
| `microsoft_iwa_contribution/` | What does Microsoft (Copilot) uniquely add to all_confirmed beyond what AEI already covers, and which additions ride on the IWA-level scoring mechanic (Microsoft scores an IWA → score gets applied to every eco_2025 task in that IWA)? Two analyses. **A. Microsoft-unique pairs** — set M = (task_norm, soc_2010) in `final_microsoft.csv`, E = pairs in `final_aei_all_usage_2026-02-12.csv`. M − E broken down by major, occupation, IWA, % physical. **B. IWA overlap** — split IWAs into AEI-only / overlap / MS-only; for overlap IWAs count "extras" (pairs in MS but not in AEI), then break extras down by major / occ / phys share. 7 PNGs + 10 CSVs. **Headlines:** 5,939 of 9,047 MS pairs (65.6%) are MS-unique; 5,917 of those (99.6%) sit in IWAs AEI ALSO touches — Microsoft's marginal contribution is volume, not breadth (AEI covers 297 IWAs vs MS's 136; 135 of MS's 136 are in overlap, only 1 MS-only). MS-unique pairs are 35.7% physical overall but bimodal at major: Production (69% phys, 585 pairs), Installation/Maint (74%, 333), Healthcare Pract (29%, 462) on top alongside cognitive Computer/Math (5%, 331), Management (9%, 354), Business/Finance (9%, 346). Top extras IWAs: "Adjust equipment to ensure adequate performance" (97% phys, 217 extras), "Measure physical characteristics of materials" (89%, 183), "Maintain current knowledge" (2%, 179, applied across 158 occs). Honest split: ~half the extras are physical-equipment IWAs where IWA-expansion introduces noise, ~half are cross-cutting cognitive IWAs where it adds legitimate coverage AEI's bottom-up observation pipeline misses. Closes loop with `weighting_config_test/physical_filter_audit.py` and the AI-share filter solution. |
| `physical_delimiter_correlation/` | How much of the part_1 paper convergence numbers are just the binary physical / non-physical task cut? **Variant A** — naive physical pct from eco_2025 only: `pct_A[occ] = Σ freq[non-phys] / Σ freq[all] × 100`, no auto-aug, ratio-of-totals at major/minor/broad/occ. **Variant B** — dashboard pipeline run with `physical_mode="exclude"` (auto-aug ON), so both numerator and denominator are non-physical-only. Three convergence heatmaps (replicating the part_1 2×2 SOC-level layout): variant A vs all comparators (4 sources + 6 configs + 6 external benchmarks), variant B sources convergence (mirror of `convergence.png`), variant B configs convergence (mirror of `convergence_configs.png`). Plus an occ-level scatter (pct_A vs all_confirmed colored by major), a raw-vs-partial ρ chart per source vs all_confirmed (controlling for pct_A on rank residuals), and pct_A by major sector. **Headlines:** variant A vs all_confirmed = 0.80 major / 0.66 occ; variant A vs AIOE Reading Compr = 0.94 major (higher than any single internal source hits AIOE in the paper); variant A vs `all_confirmed_conservative` = 1.00 major (conservative is essentially the physical filter applied to MS tasks). Partial correlation drops at occ level: Claude Browser 0.81 → 0.66, Claude API 0.79 → 0.65, Copilot 0.81 → 0.70 (most resistant), MCP 0.67 → 0.40 (most physical-explained). Variant B cross-source ρ at occ degrades sharply (Copilot × Claude Browser 0.20, MCP × Copilot 0.15) but within-AEI agreement (Browser × API) holds at 0.95 major / 0.77 occ. Eloundou benchmarks survive the physical control better than AIOE or Schaal (which are themselves heavily structured around the cognitive-vs-physical cut). 6 PNGs + 5 CSVs + report. |
| `risk_score_audit/` | Diagnostic on the `job_exposure/job_risk_scoring` 8-flag composite, framing the question of whether to import it into Part 3 of the paper. Two methodological overrides applied locally (not in `compute_ska.py` or the question script): (i) SKA AI capability uses top-10 mean instead of p95 — stricter "what AI demonstrably can do" floor matching the dashboard's per-row reference; (ii) flag 6 fires on `emp_change_pct_2024_2034 < 0` (BLS 2024–2034 employment projection from eco_2025) instead of DWS outlook in {2,3} — direct employment-trajectory signal replacing the 2-axis outlook+wages tradeoff. 248 of 923 occs have negative projected change. Four sections: (1) flag-validity diagnostics — eta² of each flag against pct_physical, job_zone, and major shows F5 (zone in 1–3) is tautological with job_zone (η² = 1.00), F7 (software > median) is heavily structural (η² ≈ 0.42 across all three cuts), F1 moderate (η² 0.12 zone, 0.36 major), F6 (emp proj < 0) much less structural than the old DWS version (η² 0.05 phys / 0.11 zone / 0.29 major vs prior 0.13/0.20/0.30 — the swap reduces structural contamination of flag 6), F2/F3/F4/F8 mostly independent; (2) SKA mechanicalness (top-10 mean) — OLS R²(ska_pct ~ pct_physical) = 0.03 (negligible), R²(ska_pct ~ job_zone) = 0.19, combined 0.22; direction on zone reversed from the upstream intuition (lower-zone occs have HIGHER SKA pct because their requirement floor is lower); (3) level-vs-trend independence — eight 2×2 contingencies: pct level × pct trend has 21% off-diag at median (φ = 0.57) / 11% at p75 (φ = 0.72); SKA × SKA tighter under top-10 mean (10% / 6% off-diag, φ = 0.80 / 0.83); cross pairings at φ ≈ 0; (4) flagging variants — nine "high-exposure" definitions. A (`pct > 50%`), B (`pct > p75`), D (`pct > 50% + trend top half`) essentially identical (Jaccard 0.94–0.98, ~230 occs each); C (`pct > 50% + emp proj < 0`) drops to 59 occs (was 83 with DWS); E (trimmed 4-flag composite) is 200 occs with 87% Jaccard with A; F (full 8-flag) is 109 occs with 46% Jaccard with A and 61% zone 1–3 (vs 30% baseline) / 44% emp proj < 0 (vs 25%). Three quad-intersect variants (pct + SKA>med + pct trend>med + emp proj < 0): G (`pct>50%`) → 43 occs, H (`pct>median`) → 62 occs, I (`pct>p75`) → 41 occs. G ≈ I (Jaccard 0.95). H is the only quad-intersect with distinct content (admits 19 mid-exposure occs in trajectory). All three force emp proj < 0 by construction and run 76–84% zone 1–3. Recommendation in the report: four defensible options in increasing strictness — A (raw pct cut), E (trimmed composite), C (pct ∩ emp proj decline), H (pct>med + 3-condition watch list). Avoid F (too structural), B/D (≈ A), G/I (≈ each other). Section 5 produces four chart variations on a focused 56-occupation set (`pct > 50% AND pct trend > med AND emp proj < 0`, with a flag for whether SKA pct > median also fires — 43 SKA-gated, 13 added without SKA). The 13 SKA-released additions are mostly Zone 4–5 educators (6 of 13: Elementary/Middle/Secondary/CTE Secondary/Foreign Lang Postsec/History Postsec), research-adjacent professionals (Geographers, Political Scientists, Survey Researchers, Transportation Planners), and Computer/Math knowledge workers (Mathematicians, Network Admins) — a forward-looking professional tier vs. the SKA-gated clerical core. Variations: 5a horizontal bar by pct desc, 5b horizontal bar by emp projection asc, 5c scatter (emp proj × pct, marker size = workers), 5d stacked bar by major. Intended for the next chat to pull into Part 3's "what to do about it" section. Side-deliverables: `focused_set.csv` (the 56 occs with tier flag), `ska_below_100_top10.csv` (250 occs with SKA top-10 mean < 100% vs 735 under p95, clean physical-vs-cognitive-ceiling split). 10 PNGs + 9 CSVs. |

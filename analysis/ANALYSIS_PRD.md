...
| `risk_score_audit/` | Diagnostic on the `job_exposure/job_risk_scoring` 8-flag composite, framing the question of whether to import it into Part 3 of the paper. Four sections: (1) flag-validity diagnostics — eta² of each flag against pct_physical, job_zone, and major shows F5 (zone in 1–3) is tautological with job_zone (η² = 1.00), F7 (software > median) is heavily structural (η² ≈ 0.42 across all three cuts), F1/F6/F2 are moderately structural, and F3/F4/F8 (the trend and auto-aug flags) are mostly independent of structure; (2) SKA mechanicalness — OLS R²(ska_pct ~ pct_physical) = 0.03 (negligible), R²(ska_pct ~ job_zone) = 0.18, combined 0.21, with the *direction* on zone reversed from the upstream intuition (lower-zone occs have HIGHER SKA pct because their requirement floor is lower); (3) level-vs-trend independence — eight 2×2 contingencies (4 pairings × median+p75): pct level × pct trend has 21% off-diag at median (φ = 0.57) and 11% off-diag at p75 (φ = 0.72); SKA level × SKA trend is tighter (11% / 8% off-diag, φ = 0.78 / 0.80); the two cross pairings are at φ ≈ 0; (4) flagging variants — nine "high-exposure" definitions compared by set size, structural composition, and Jaccard. A (`pct > 50%`), B (`pct > p75`), and D (`pct > 50% + trend top half`) are essentially identical (Jaccard 0.94–0.98, ~230 occs each); C (`pct > 50% + outlook 2–3`) drops to 83 occs; E (trimmed 4-flag composite: pct, pct-trend, outlook, auto-aug) lands at 208 occs with 88% Jaccard with A; F (full 8-flag) is 115 occs with only 46% Jaccard with A and 59% in zones 1–3 (vs 30% baseline) / 53% outlook 2–3 (vs 35%). Three quad-intersect variants (pct + SKA>med + pct trend>med + outlook 2–3) at three pct thresholds: G (`pct>50%`) → 53 occs, H (`pct>median`) → 95 occs, I (`pct>p75`) → 52 occs. G ≈ I (Jaccard 0.98 — pct>50% and pct>p75 are nearly the same threshold for this dataset). H is the only one of the three with distinct content: by dropping the pct floor to the median (which is below 50%), it picks up 42 mid-exposure occupations meeting the SKA + trend + outlook conditions — a "trajectory" / watch-list cut. All three force outlook 2–3 by construction (100%) and run 75–83% zone 1–3. Recommendation in the report: four defensible options in increasing strictness — A (raw pct cut), E (trimmed composite), C (pct ∩ outlook), H (pct>med + 3-condition intersect for the forward-looking watch list). Avoid F (too structural), B/D (essentially A), and G/I (essentially each other). 6 PNGs + 7 CSVs. |

...
The previous analysis ("AI Transformative Potential") identified where AI *could* change things — the gap between capability and adoption. This question asks a harder, more policy-relevant question: **where are jobs actually at risk of disappearing?**
...
|------|-----------|---------------|
| **High Risk** | >= 60% | Majority of task value is already being done by AI. Job's existence as currently defined is in question. |
| **Moderate Risk** | 40–60% | Significant AI overlap. Job likely restructures — core changes but doesn't disappear. |
...
Almost half of occupations (457 of 923) have less than 20% task exposure from current AI usage. But the moderate risk tier alone covers 35.9 million workers — nearly a quarter of the national workforce — in occupations where 40–60% of the task value is AI-overlapping.

![Job Elimination Risk: AI Task Exposure vs Employment](figures/scatter_risk_vs_employment.png)

The scatter plot above maps every occupation by its % tasks affected (X-axis) against total employment (Y-axis, log scale). The color coding shows risk tiers. The upper-right quadrant — high exposure AND large workforce — is where the policy concern concentrates. Note that the 9 high-risk occupations (red dots) are mostly mid-size by employment (30K–430K), while the truly massive occupations (1M+ workers) cluster in the moderate and restructuring tiers.

---

## 2. The High-Risk Tier: 9 Occupations at 60%+ Task Exposure
...
![Largest High-Risk Occupations by Employment](figures/high_risk_by_employment.png)
...
**Business Intelligence Analysts** (69% exposed, 122K workers) and **Financial Quantitative Analysts** (61%, 93K) represent the analytical end of business and finance — building models, running queries, interpreting datasets. These are also among the highest-paid occupations in the high-risk tier ($100K+ median wage), meaning the wage impact per worker is substantial.

**Poets, Lyricists and Creative Writers** have the single highest exposure (70%) but a small workforce (45K). This is the occupation most often cited in public discourse about AI replacing creative work, and the data supports the concern — at least in terms of task overlap with demonstrated AI usage.
...
## 4. Risk Distribution by Major Category

Not all sectors are equally exposed. The chart below shows what share of each major category's occupations fall in each risk tier:

![Risk Tier Distribution by Major Category](figures/tier_distribution_by_major.png)

### Highest concentration of at-risk occupations

**Computer and Mathematical Occupations** have the most exposure: roughly 60% of occupations in this category are in the moderate or high-risk tiers. This isn't surprising — these are the occupations most directly overlapping with AI's analytical and code-generation capabilities.

**Business and Financial Operations** follows closely, with the majority of occupations in the restructuring-or-higher tiers. Three of the 9 high-risk occupations (Market Research Analysts, Financial Quant Analysts, Search Marketing Strategists) come from this category.

**Arts, Design, Entertainment, Sports, and Media** has a notable high-risk presence from creative writing occupations, plus a substantial moderate-risk tier from design and media roles.

**Educational Instruction and Library Occupations** is interesting — one high-risk occupation (Instructional Coordinators) and a substantial moderate tier, reflecting the heavy AI usage in educational contexts documented in the transformative potential analysis.
...
![Employment by Risk Tier and Major Category](figures/employment_by_tier_major.png)
...
**The results are highly stable.** Only 34 of 923 occupations (3.7%) change tiers when switching methods. The high-risk tier is nearly identical:

| Method | High-Risk Occupations |
|--------|---------------------|
| Value (primary) | 9 |
| Time (frequency) | 7 |

**7 occupations are high-risk under both methods** — these are the most robust findings:
...
5. **Method sensitivity is low: 96.3% of occupations stay in the same tier regardless of weighting methodology.** Seven of 9 high-risk occupations are stable across both Value and Time methods.
...
| `results/stable_high_risk_both_methods.csv` | 7 occupations high-risk under both methods |

### Figures
| File | Description |
|------|-------------|
| `figures/scatter_risk_vs_employment.png` | Scatter: % tasks affected vs employment, colored by tier |
| `figures/high_risk_by_employment.png` | Bar: high-risk occupations by employment |
| `figures/high_risk_by_pct.png` | Bar: high-risk occupations by % tasks affected |

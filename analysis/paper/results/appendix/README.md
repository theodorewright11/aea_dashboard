...
- **capability_vs_adoption_all_occs** — companion to `adoption_friction_scatter`, 3×2 panel chart across all 923 occupations. Top two rows scatter four capability properties (Schaal ag, Schaal da, our s, our d) against `all_confirmed` exposure; bottom row scatters our adoption frictions (r, df). Blue dot ramp for capability, gold for adoption. Per-occupation property means are unweighted across all of the occupation's tasks. Headline: capability props correlate at ρ +0.55 to +0.68 across all occs (they ride the phys/non-phys split); adoption props barely discriminate at the all-occ level (ρ −0.19 for `r`, −0.17 for `df`). This is why the friction signal only emerges once `adoption_friction_scatter` restricts to non-physical occupations — the two charts together show the framework's two-layer structure.

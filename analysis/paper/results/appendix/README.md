...
- **adoption_friction_scatter** — two-panel occupation-level scatter testing the Section 3 adoption-layer prediction that risk (`r`) and deployment friction (`df`) discriminate exposure once capability is roughly held constant. Restricted to the ~409 occupations whose unique-task share of physical tasks is `< 33%` (Part 2's Non-Physical bucket); one dot per occupation. X-axis is the unweighted mean of the friction property across that occupation's non-physical tasks (from `data/final_eco_2025_with_task_properties.csv`, the same 12-property file `audit_task_properties` uses); y-axis is % Tasks Exposed under `all_confirmed`. Dot color mirrors the y value (light → dark blue ramp). Dashed OLS fit plus a Spearman ρ inset per panel. Headline: ρ = −0.48*** for `r`, −0.40*** for `df`, both n = 409, p &lt; .001. Notes: freq×emp weighting was tried and washes the cross-occ variance out; major-level rollups within non-phys give ρ ≈ 0 — the property discrimination lives at occupation level here.

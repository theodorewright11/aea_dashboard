...
│   │                              Frey-Osborne, Autor, Tomlinson (Copilot)).
...
- **Focused set is ~10–13% of state employment, not a handful of niche occupations**: the SKA-gated "Most At Risk" set used by `risk_score_5f` and `state_exposure_at_risk` (currently n≈44, but rebuilt from `_build_focused_set` each run so the exact count can shift with flag data) includes very-large-employment declining roles (Retail Salespersons 3.8M, Customer Service Reps 2.7M, Office Clerks 2.5M, Secretaries 1.7M, Bookkeepers 1.5M). Together they cover ~20M U.S. workers (~12–13% of total emp). When computing per-state shares, the headline values land in the 4–12% range — that's not a bug, it's the structural shape of the focused set. Do not under-scale the chart's x-axis on this expectation. The appendix `state_clusters_each_ranked` chart pulls its n via `len(_load_focused_set())` so its label stays in sync with Part 3's `risk_score_5f`.

...
│   │   ├── README.md        — Question description, methodology, findings (with inline figures)
│   │   ├── run.py           — Generates CSVs + figures
│   │   ├── dashboard.md     — Dashboard settings to reproduce charts on the website
│   │   ├── figures/         — Key figures embedded in README.md (committed to git)
...
6. Have `run.py` copy key figures to a committed `figures/` directory (outside `results/`) and embed them inline in `README.md` using `![alt](figures/filename.png)` syntax
7. Add the question's findings to `report/report.md`

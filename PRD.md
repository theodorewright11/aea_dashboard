...
- **Where you rank** — gradient-filled rank bars for the economy (out of 923), major (out of N occs in major), minor, broad. Per scope: % Tasks rank with workers/wages ranks shown beneath. Plus an **intensity rank** sub-card showing this occupation's per-task usage ratio rank (Σ pct ÷ Σ freq×emp, bias-corrected for source asymmetries) within the economy and its major.
...
- **Tasks table** — all unique tasks for the occupation, color-coded by max(AEI Conv, AEI API, Microsoft). Each task row shows a tier dot, task text, four per-source mini-bars (Conv, Agent, Copi, MCP), and a max score. Tasks are grouped into three collapsible color-bucket sections ("AI does this well" / "AI helps with this" / "Mostly still you") so users can scroll past the lower tiers. Each row is click-to-expand to reveal the top-5 MCP servers that match the task, with names, ratings, URLs, and descriptions pulled from `data/mcp_titles_desc.csv`. AEI API reflects agentic capability specifically (a callout near the table explains the conv/agentic distinction).
- **Work activities table** — tab selector for GWA / IWA / DWA. Each table is grouped into the same three color-bucket collapsible tiers. Per row: name, # tasks (in occ), per-source averages (Conv / API / MS / MCP) rolled up from the occupation's tasks, plus four economy-wide columns (eco % tasks, eco workers, eco wages, eco mean auto-aug) — each economy value shows its rank within all GWAs / IWAs / DWAs in the economy beneath the value. This lets users see not just how AI hits this WA for their occupation but how the WA ranks across the entire economy.

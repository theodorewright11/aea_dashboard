...
- **Hero card** — occupation name, full SOC hierarchy (broad/minor/major), Job Zone chip with interpretation, Outlook chip with interpretation, total task count, and an **Exposure tier chip** (Low / Mod-Low / Mod-High / High Exposure) with the 0–10 score, rendered as a half-circle gauge. Clicking "Why?" expands an 8-flag breakdown showing which signals fired (lifted from the analysis bucket's `job_risk_scoring` formula). All UI strings say "Exposure" — internal payload field stays as `risk` for back-compat.
- **Headline numbers (KPI row)** — % Tasks Affected (with embedded sparkline of the all_confirmed trend), Workers Affected, Wages Affected, and **AI adoption rank** (renamed from "Intensity rank") which describes how much workers in this occupation already use AI relative to their task load (Σ pct ÷ Σ freq×emp, bias-corrected for source asymmetries — same underlying intensity metric).
- **Trend sparkline** — embedded inside the first KPI card, with first-to-last % delta visible.
- **Where you rank** — gradient-filled rank bars for the economy (out of 923), major (out of N occs in major), minor, broad — showing where THIS occupation sits within each scope on % tasks affected. Workers/wages ranks shown beneath each bar.
- **Sector chain** — three stacked entries (Major / Minor / Broad) for the SOC categories the occupation belongs to. Each entry shows its name, % tasks affected, workers affected, wages affected, and the rank of that SOC category within the economy on each metric (e.g., the major's rank among 22 majors, the minor's rank among ~98 minors, the broad's rank among ~456 broads). Drawn from the new `sector_chain` payload key.
- **SKA gap section** — three subsections (Skills, Knowledge, Abilities), each collapsible. Within each subsection rows are further grouped into the three color-bucket collapsible tiers. Per row (importance ≥ 3 only): importance, level, occ score (importance × level), AI top-10 reference (mean of top-10 ai_product values for that element), gap, AI as % of need, neutral 3-tier color. Sorted with biggest AI lead at top within each tier. Section header shows the four ratio-of-sums summaries (Overall / Skills / Knowledge / Abilities).
- **Tasks table** — all unique tasks for the occupation, color-coded by max(AEI Conv, AEI API, Microsoft). Tasks are grouped into three collapsible color-bucket sections ("AI does this well" / "AI helps with this" / "Mostly still you") so users can scroll past the lower tiers. A header row labels columns; per row: tier dot + task text, four per-source mini-bars labeled **Claude Conv / Claude API / Copilot / MCP** (each showing its 0–5 numeric value above the bar), **Freq×Imp×Rel** (the task's value weight — what the dashboard's "value" method uses), **Rank in occ** by that value weight (color-intensity coded — top-ranked tasks render bold/dark, bottom render muted, so the load-bearing tasks pop visually), and **Max AI** colored by tier. A chevron on the right indicates that the row is expandable; clicking expands to show the top-5 MCP servers (only when matches exist) with names, ratings, URLs, and descriptions.
- **Work activities section** — tab selector for GWA / IWA / DWA. Each WA is rendered as a card (not a row) with three stacked sections: header (tier dot + name + n_tasks), per-source mini-bars (Claude Conv / Claude API / Copilot / MCP, each with 0–5 value above the bar), and a horizontal pill strip of economy-wide stats (Eco %, Workers, Wages, Auto) — each pill carrying its `#rank/total` inline. Cards are still grouped into the three collapsible color-bucket tiers.
- **Software commodities** — instead of listing individual softwares, this card aggregates by O*NET commodity category and shows: commodity title, count of softwares this occupation uses in that commodity, and the commodity's economy-wide rank by average % tasks affected. Sorted by commodity rank ascending so the most-AI-exposed commodities surface first.
- **Similar occupations** — top-5 nearest by L1 distance over the occupation's SKA profile (importance × level vector across all S+K+A elements with imp ≥ 3 in either occupation). Each row shows occupation, sector, **Exposure profile** (a tier chip with the 0–10 score plus an 8-dot strip showing which exposure flags are lit), % tasks affected, median wage, job zone, outlook rating, and the SKA distance.
...
- **Collapsibility** — every body card has a click-to-collapse header (chevron + section title). Within Tasks, SKA, and Work Activities, rows are additionally grouped into collapsible color-bucket tiers (high/mid open by default, low/none closed by default) so long lists are scrollable without scroll-jumping past what matters.

...
From `questions/job_exposure/job_risk_scoring/`. Weighted 7-factor composite, computed per occupation against the primary config (`all_confirmed`).

**E2.13.1 — Flags and weights:**

| Flag | Weight | Condition |
|---|---|---|
| 1 | 2 | `pct_tasks_affected > median(pct_tasks_affected)` |
| 2 | 2 | `overall_gap > median(overall_gap)` (SKA) |
| 3 | 2 | `pct_delta > 0` AND `pct_delta > median(pct_delta)` (median over ALL growth, incl. negative) |
| 4 | 2 | `ska_delta > 0` AND `ska_delta > median(ska_delta)` |
| 5 | 1 | `job_zone ∈ {1, 2, 3}` |
...
$$
S_o = 2(F_{1,o} + F_{2,o} + F_{3,o} + F_{4,o}) + 1(F_{5,o} + F_{6,o} + F_{7,o})
$$

Maximum possible $S = 11$.

**E2.13.3 — Exposure gate:**
$$
\text{tier}_o = \begin{cases}
\text{low} & S_o \in [0, 3] \\
\text{moderate} & S_o \in [4, 7] \\
\text{high} & S_o \in [8, 11] \text{ AND } \text{pct\_tasks\_affected}_o \geq 33 \\
\text{moderate} & S_o \in [8, 11] \text{ AND } \text{pct\_tasks\_affected}_o < 33 \text{ (exposure-gated)}
\end{cases}
$$
...
**Not a distance metric.** This is a **rectified element-wise gap sum** — not cosine, Euclidean, or anything symmetric. Only the elements where the low-risk group scores *above* the high-risk group contribute (those are the skills the high-risk worker would need to learn); elements where the high-risk group is already above are dropped from the sum.
...
**E2.18.1 — Per-row exposure weight.** Every (occ, category) row inherits a weight equal to its occupation's exposure × workforce size:

$$
w^{\text{tech}}_{o, c} \;=\; \text{pct\_tasks\_affected}_o \cdot e_o
$$

Note: `pct_tasks_affected` is left on its 0–100 scale here (not divided by 100), because these weights are only used relatively for ranking, not as worker counts.

**E2.18.2 — Per-category economy rollup.** For each Commodity Title $c$:

$$
\text{total\_exposure\_weight}_c \;=\; \sum_{o \,:\, (o,c) \in \mathcal{T}} w^{\text{tech}}_{o, c}
$$

$$
\text{n\_occs}_c \;=\; \left| \{o : (o,c) \in \mathcal{T}\} \right|, \qquad \text{n\_entries}_c \;=\; \left| \{(o,c') \in \mathcal{T} : c' = c\} \right|
$$

Categories are then sorted descending by `total_exposure_weight`. The top-25 drive the "Top tech categories by exposure-weighted presence" bar chart.

**E2.18.3 — Per-sector tech penetration.** For each major SOC group $G$ and each top-25 category $c$:

$$
\text{pct\_occs}_{G, c} \;=\; \frac{\left|\{o \in G : (o, c) \in \mathcal{T}\}\right|}{\left|\{o \in G : \exists c'\,(o, c') \in \mathcal{T}\}\right|} \times 100
$$

This is an **unweighted** share: the percent of occupations in sector $G$ (among those that appear at all in `technology_skills_v30.1.csv`) that list technology category $c$. It drives the `tech_major_heatmap.png` figure.

**Why the two metrics differ.** `total_exposure_weight` (E2.18.2) weights a category by how many workers are in the exposed occupations that use it — "which tech is most entangled with AI-exposed employment." `pct_occs` (E2.18.3) is a raw penetration rate — "which tech is most widespread in a sector," ignoring both employment size and exposure level. A category can rank high on one and low on the other.
...
| `job_risk_scoring/` | `all_confirmed` primary; `tech_skills_simple.csv`; `job_zone`, `dws_star_rating` from §1.1 | **E2.12.1–7** (SKA + SKA trend), **E2.13.1–4** (7-flag composite) |
| `worker_resilience/` | `all_confirmed`; SKA data | **E2.12.1–6**, sorted ascending by `gap` |
| `pivot_distance/` | `pivot_distance_inputs.csv` from `job_risk_scoring`; SKA (skills+knowledge only); `job_zone` | **E2.14.1–4** (per zone, both `all_confirmed` and `all_ceiling`) |
...
| `automation_opportunity/` | **E2.12** (SKA gap, AI-leads quadrant Q1), plus **E2.17.1** for the adoption gap axis, intersected with **E2.13** tiers for "transformation signal" |
...
| `skills_landscape/` | Two distinct parts. **SKA side:** **E2.12.3–4** at the economy level (AI-leads vs human-leads elements) + **E2.12.6** averaged by major sector across **all five configs** for the major × config heatmap. **Tech side:** **E2.18.1–2** for the economy-level exposure-weighted category ranking and **E2.18.3** for the per-sector penetration heatmap (both using `all_confirmed`). |
